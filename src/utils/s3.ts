...
export const uploadObjects = async (images: string[]) => {
  const upload_params = images.map((name) => ({
    Bucket: env.S3_NAME,
    Key: name,
  }));
  const upload_commands = upload_params.map(
    (params) => new PutObjectCommand(params),
  );
